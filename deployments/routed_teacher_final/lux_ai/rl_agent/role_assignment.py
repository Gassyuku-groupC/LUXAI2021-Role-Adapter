...
class RoleCityBiasParams:
    ...
    # Retained only so existing checkpoints load without changing parameter keys.
...
def classify_city_specializations(
    ...
    return {
        city_id: _replace_city_role(
            spec,
            _city_role_for_id(city_id, depot_id, manufacturing_id),
            _city_reason_for_id(city_id, depot_id, manufacturing_id, spec),
        )
...
def _city_role_for_id(city_id: str, depot_id: str, manufacturing_id: Optional[str]) -> str:
